// Server-authoritative STL volume. Pure, synchronous. Volume =
// |Σ signed-tetrahedron(v0,v1,v2)| over all triangles. Units = mm³.
...
function parseAscii(buf: Buffer): StlVolumeResult {
  ...
  return { volumeMm3: Math.abs(vol), triangleCount: verts.length / 3, format: 'ascii' };
}

export function stlVolumeMm3(buffer: Buffer): StlVolumeResult {
  if (!buffer || buffer.length === 0) throw new Error('Empty STL buffer');
