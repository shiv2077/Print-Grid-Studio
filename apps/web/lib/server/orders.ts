...
export async function addOrderFile(data: NewOrderFile) {
  const [row] = await db.insert(orderFiles).values(data).returning();
...
export async function findOrderByRazorpayId(razorpayOrderId: string): Promise<OrderRow | null> {
  const [row] = await db.select().from(orders).where(eq(orders.razorpayOrderId, razorpayOrderId)).limit(1);
  return row ?? null;
}
